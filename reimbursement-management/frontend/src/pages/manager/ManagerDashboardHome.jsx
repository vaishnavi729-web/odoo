...
export default function ManagerDashboardHome() {
  ...
  const pendingApprovals = expenses.filter(e => 
    e.status === 'pending' && 
    e.approval_steps.some(s => s.status === 'pending' && (s.approver_id === user.id || (!s.approver_id && s.approver_role === 'manager')))
  );
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 2fr) minmax(0, 1fr)', gap: '1.5rem' }}>
        <div className="glass-card" style={{ padding: '1.5rem' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '1.25rem' }}>
            ...
            <Link to="/manager/approvals" style={{ fontSize: '0.8125rem', color: 'var(--primary)', fontWeight: 600, textDecoration: 'none', display: 'flex', alignItems: 'center', gap: '0.25rem' }}>
